...
//		fos.write(intArrayToByteArray(timeSigEvent));
	...
	public void playSequence(boolean[][] b, Sound sound, int tempo) {

		ArrayList<Integer> notesPlaying = new ArrayList<Integer>();

		for (int i = 0; i < b[0].length; i++) {
			for (int j = 0; j < b.length; j++) {
				if (b[j][i]) {
					this.noteOn(0, sound.getMidiNote(b.length-1-j), 127);
					notesPlaying.add(b.length-1-j);
				}
			}
			// Om ingen not spelades
			if (notesPlaying.size() == 0) {
				this.noteOn(0, 0, 0);
				this.noteOff(4, 0);
			}

			//Om en eller flear noter spelades
			else {
				this.noteOff(4, sound.getMidiNote(notesPlaying.get(0)));
				// Stoppar alla noter som spelas just nu
				for (int j = 1; j < notesPlaying.size(); j++) {
					this.noteOff(0, sound.getMidiNote(notesPlaying.get(j)));
				}
				notesPlaying.clear();
			}
		}
